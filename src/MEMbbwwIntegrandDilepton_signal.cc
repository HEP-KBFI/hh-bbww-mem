...
double MEMbbwwIntegrandDilepton_signal::Eval(const double* x, int & countEval) const
{
  if ( !(countEval > 0) ) countEval = 0;
  if ( verbosity_ >= 2 )
  {
  ...
  if ( madgraphIsInitialized_ ) {
    bool isDebug = (verbosity_ >= 2 || 1 > 0) && countEval == 0 && prob_ME == -1.;
