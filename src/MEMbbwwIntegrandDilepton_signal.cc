  ...
  , chargedLeptonPermutation_(kPermutationUndefined)
{
  ...
  intIntBounds_upper_[5] = +TMath::Pi();
  ...
  //   6) outgoing neutrino
  //   7) outgoing lepton of positive charge
					   ...
					   double measuredMEtPx, double measuredMEtPy, const TMatrixD& measuredMEtCov)
{
  ...
  // Standard Model (SM) cross section for (non-resonant) HH productioa @ 13 TeV center-of-mass energy
...
double MEMbbwwIntegrandDilepton_signal::Eval(const double* x) const
{
  ...
  if ( verbosity_ >= 2 ) {
    ...
    LorentzVector sumP4 = trueBJet1P4 + trueBJet2P4 + trueChargedLeptonPlusP4 + trueNuP4 + trueChargedLeptonMinusP4 + trueAntiNuP4;
    ...
    LorentzVector sumP4_ztm = trueBJet1P4_ztm + trueBJet2P4_ztm + trueChargedLeptonPlusP4_ztm + trueNuP4_ztm + trueChargedLeptonMinusP4_ztm + trueAntiNuP4_ztm;
