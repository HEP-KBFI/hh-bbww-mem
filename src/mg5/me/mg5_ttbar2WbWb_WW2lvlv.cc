...
#include "hhAnalysis/bbwwMEM/interface/mg5/helamps/HelAmps_sm_ttbar2WbWb_WW2lvlv.h"

using namespace MG5_sm_ttbar2WbWb_WW2lvlv;
...
void mg5_sm_ttbar2WbWb_WW2lvlv::initProc(const string & param_card_name)
{
  // Instantiate the model class and set parameters that stay fixed during run
  pars = Parameters_sm_ttbar2WbWb_WW2lvlv::getInstance();
...
void mg5_sm_ttbar2WbWb_WW2lvlv::sigmaKin()
{
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_1_gg_ttx_t_wpb_wp_epve_tx_wmbx_wm_emvex();
...
double mg5_sm_ttbar2WbWb_WW2lvlv::sigmaHat()
{
...
double mg5_sm_ttbar2WbWb_WW2lvlv::matrix_1_gg_ttx_t_wpb_wp_epve_tx_wmbx_wm_emvex()
{
  int i, j;
  // Local variables
  const int ncolor = 2;
