...
MEMIntegratorVEGAS::MEMIntegratorVEGAS(unsigned numCallsGridOpt, unsigned numCallsIntEval, double maxChi2, unsigned maxIntEvalIter)
  : integrand_(0),
...
void MEMIntegratorVEGAS::setIntegrand(MEMIntegratorBase::gPtr_C g, const double* xl, const double* xu, unsigned d)
{
  numDimensions_ = d;
...
void MEMIntegratorVEGAS::integrate(MEMIntegratorBase::gPtr_C g, const double* xl, const double* xu, unsigned d, double& integral, double& integralErr)
{
  setIntegrand(g, xl, xu, d);
  
  if ( !integrand_ ) {
    std::cerr << "<MEMIntegratorVEGAS>: No integrand function has been set yet --> ABORTING !!\n";
    assert(0);
  }
