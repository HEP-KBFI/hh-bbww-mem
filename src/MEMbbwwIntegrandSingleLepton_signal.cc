...
double MEMbbwwIntegrandSingleLepton_signal::Eval(const double* x, int & countEval) const
{
  if ( verbosity_ >= 2 )
  {
  ...
  if ( madgraphIsInitialized_ )
  {
    if ( measuredChargedLepton_->charge() > 0 )
    {
      ...
      me_madgraph_chargedLeptonPlus_.sigmaKin(isDebug);
    ...
    else if ( measuredChargedLepton_->charge() < 0 )
    {
      ...
      me_madgraph_chargedLeptonMinus_.sigmaKin(isDebug);
  ...
  const double fudgeFactor               = 1.e+65;
  const double fudgeFactor_missingBJet   = 1.e+58;
  const double fudgeFactor_missingWJet   = 1.e+60;
  const double fudgeFactor_missingBnWJet = 1.e+52;
