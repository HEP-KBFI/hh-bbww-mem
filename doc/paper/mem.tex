...
The matrix element method (MEM) computes probability densities $w_{i}(\vecy|\vecyhat)$
...
This yields~\cite{CMS_AN_2013_313}:
\begin{equation}
...
we follow the approach of Ref.~\cite{CMS_AN_2013_313}.
...
Following Ref.~\cite{CMS_AN_2013_313}, we assume that the resolution on the transverse momentum components of $\rho$ follows a two-dimensional normal distribution:
\begin{equation}
...
\end{equation}
The matrix $V$ is computed event-by-event, using the $\MET$-significance algorithm~\cite{CMS_AN_2010-400}.
Following Ref.~\cite{CMS_AN_2013_313}, we replace the $\delta$-functions 
$\delta( \pXhat^{\rho} + \sum_{i}^{6} \pXhat^{(i)})$ and $\delta( \pYhat^{\rho} + \sum_{i}^{6} \pYhat^{(i)})$
...
\section{Analytic transformations specific to the signal hypothesis}
...
\section{Analytic transformations specific to the background hypothesis}
...
and the masses of the $\Pbottom\ellPlusnu$ and $\APbottom\ellMinusnu$ systems are equal to the top quark mass of $m_{\top} = 173.0$~\GeV~\cite{PDG}.
...
\begin{align}
w_{1}(\vecy|\vecyhat) 
 = & \frac{(m_{\top} \, \Gamma_{\top})^{2} \, (m_{\PW} \, \Gamma_{\PW})^{2}}{2^{21} \, \pi^{14} \, \sigma \, E_{\Plepton^{+}} \, E_{\Plepton^{-}} \, E_{\Pbottom} \, E_{\APbottom}} \, \int \,
...
\frac{1}{\Ehat_{\ellMinusnu} \, \left( 1 - \frac{\betahat_{\ellMinusnu}}{\betahat_{\APbottom}} \, \cos\sphericalangle(\vecehat_{\ellMinusnu},\vecehat_{\APbottom}) \right)} \nonumber \\
 = & \frac{(m_{\top} \, \Gamma_{\top})^{2} \, (m_{\PW} \, \Gamma_{\PW})^{2}}{2^{25} \, \pi^{14} \, \sigma \, s \, 
...
with:
\begin{eqnarray}
\Ehat_{\Pbottom} & = & \frac{a_{\top} \, \Delta_{m_{\top}}
 + |b_{\top}| \, \sqrt{\Delta_{m_{\top}}^{2} - (a_{\top}^{2} - b_{\top}^{2}) \, m_{\Pbottom}^{2}}}{a_{\top}^{2} - b_{\top}^{2}} \nonumber \\
\Ehat_{\APbottom} & = & \frac{a_{\antitop} \, \Delta_{m_{\top}}
 + |b_{\antitop}| \, \sqrt{\Delta_{m_{\top}}^{2} - (a_{\antitop}^{2} - b_{\antitop}^{2}) \, m_{\Pbottom}^{2}}}{a_{\antitop}^{2} - b_{\antitop}^{2}} \nonumber \\
...
where:
\begin{eqnarray}
\Delta_{m_{\top}} & = & \frac{m_{\top}^{2} - m_{\Pbottom}^{2} - m_{\PW}^{2}}{2} \nonumber \\
a_{\top} & = & \Ehat_{\ellPlusnu} \nonumber \\
b_{\top} & = & \sqrt{\Ehat_{\ellPlusnu}^{2} - m_{\PW}^{2}} \, 
...
a_{\antitop} & = & \Ehat_{\ellMinusnu} \nonumber \\
b_{\antitop} & = & \sqrt{\Ehat_{\ellMinusnu}^{2} - m_{\PW}^{2}} \, 
